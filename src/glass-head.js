...
const fragmentShader = /* glsl */ `
  ...
  void main() {
    ...
    // но живёт на рельефе, а не на плоскости картинки:
    // 1) координата искривлена картой глубины — фронт обтекает нос и оправу;
    // 2) яркость зависит от нормали: грани, повёрнутые к полосе, вспыхивают,
    //    обратные склоны остаются тёмными — свет ползёт ПО форме, не сквозь
    vec2 axis = vec2(0.927, -0.374);
    float t = dot(vUv - 0.5, axis) + 0.5 - vDepth * 0.16;
    float d = t - uSweep;
    float band = exp(-pow(d / 0.035, 2.0)) + 0.18 * exp(-pow((d - 0.06) / 0.08, 2.0));
    float sgn = clamp(-d / 0.04, -1.0, 1.0);              // с какой стороны полоса
    float facing = clamp(dot(n.xy, axis * sgn) * 2.2, 0.0, 1.0);
    band *= 0.25 + 0.75 * facing;
    col += band * uGlare * vec3(0.96, 0.98, 1.05) * 2.0 * (0.10 + 0.90 * lum) * sil;
